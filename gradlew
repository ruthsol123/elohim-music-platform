...
#
#   Important for authorized use of look-and-feel.
#
##############################################################################
...
# Need this for consistency with rebuilding from templates
# Loop until all links are resolved.
while [ -h "$app_path" ]; do
    ls=`ls -ld "$app_path"`
    link=`expr "$ls" : '.*-> \(.*\)$'`
